...
function RefreshCityBanner(cityBanner, iActiveTeam, iActivePlayer)
	...
	if city ~= nil then
		...
		if (isActivePlayerCity) then
			if (city:IsPuppet() and not (player.MayNotAnnex ~= nil and player:MayNotAnnex())) then
		...
		if (controls.CityProductionName) then
			...
			if (controls.CityBannerLeftBackground) then
				if (isBNW and city:IsProductionProcess()) then
		...
		if( controls.MinorIndicator and controls.StatusIcon ) then
			...
			if( pOriginalOwner:IsMinorCiv() ) then
				...
				controls.MinorIndicator:SetColor( originalColor );
					...
					SimpleCivIconHookup(pOriginalOwner:GetAlly(), 32, controls.MinorIndicator);  
...
function OnBannerClick( x, y )
	...
	if plot then
		...
		if playerID == Game.GetActivePlayer() then
			-- Puppets are special
			if (plot:GetPlotCity():IsPuppet() and not (player.MayNotAnnex ~= nil and player:MayNotAnnex())) then
				local popupInfo = {
